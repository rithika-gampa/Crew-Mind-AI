...
function getProviderState() {
  const primaryProvider = String(process.env.PRIMARY_PROVIDER || "xai").trim().toLowerCase();
  const backupProviders = String(process.env.BACKUP_PROVIDERS || "groq,gemini")
    .split(",")
  ...
  return { primaryProvider, backupProviders, providerConfig };
}

function availableProviders() {
  const { primaryProvider, backupProviders, providerConfig } = getProviderState();
  const configured = [primaryProvider, ...backupProviders];
...
function providerLabel(provider) {
  return getProviderState().providerConfig[provider]?.label || provider;
}

function providerModel(provider) {
  return getProviderState().providerConfig[provider]?.model || "unknown";
}

function providerSummary() {
  const { primaryProvider, backupProviders } = getProviderState();
  ...
  return {
    ...
    primaryProvider,
    backupProviders,
    chainText: configuredProviders.length
      ? configuredProviders.map((provider) => `${providerLabel(provider)} (${providerModel(provider)})`).join(" -> ")
      : "not configured",
    primaryText: `${providerLabel(primaryProvider)} (${primaryProvider})`,
    backupText: backupProviders.length
      ? backupProviders.map((provider) => `${providerLabel(provider)} (${provider})`).join(" -> ")
      : "none",
  };
}
...
async function xaiMessage({ system, prompt, maxTokens = 600, stream = false }) {
  ...
  const response = await fetch("https://api.x.ai/v1/chat/completions", {
    ...
    headers: {
      ...
      Authorization: `Bearer ${providerConfig.xai.key}`,
    },
    body: JSON.stringify({
      model: providerConfig.xai.model,
      messages: [
...
async function groqMessage({ system, prompt, maxTokens = 600, stream = false }) {
  ...
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${providerConfig.groq.key}`,
    },
    body: JSON.stringify({
      model: providerConfig.groq.model,
      messages: [
        { role: "system", content: system },
        { role: "user", content: prompt },
      ],
...
async function geminiMessage({ system, prompt, maxTokens = 600, stream = false }) {
  ...
  const endpoint = stream
    ? `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(providerConfig.gemini.model)}:streamGenerateContent?alt=sse`
    : `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(providerConfig.gemini.model)}:generateContent`;

  const response = await fetch(endpoint, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-goog-api-key": providerConfig.gemini.key,
    },
    body: JSON.stringify({
      systemInstruction: {
        parts: [{ text: system }],
...
async function handleClaudeStreamRequest(request, response) {
  try {
